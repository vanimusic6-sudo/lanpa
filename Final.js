(function () {
    ...
    /* ===== СТИЛИ (живые колонки + въезд досье; кавычка-символ, без escape) ===== */
    function injectCSS() {
        ...
        s.innerHTML =
            ...
            '.sw-col{flex:1;background:rgba(255,255,255,.04);padding:15px 17px;border-radius:12px;border:1px solid rgba(255,255,255,.06);transition:transform .2s ease,border-color .2s ease,background .2s ease}' +
            '.sw-col.focus,.sw-col:hover{transform:translateY(-2px);border-color:rgba(255,255,255,.18);background:rgba(255,255,255,.06)}' +
    ...
    function analyze(movie) {
        return Promise.all([ loadCredits(movie), loadMeta(movie) ]).then(function(arr) {
            ...
            if (rating >= 8.0 && votes >= 2000 && yr > 0 && yr <= now - 3) addP('🏛 культовый статус', 18);
            if (yr >= now - 1 && votes >= 200) addP('🔥 на хайпе', 10);
            if (votes > 0 && votes < 30) addC('❓ оценок слишком мало', 8);
            ...
            if (runtime > 0 && runtime <= 120) addP('🕐 можно посмотреть за вечер (' + runtime + ' мин.)', 6);
            ...
            if (runtime > 180) addC('⌛ тяжело посмотреть за раз (' + runtime + ' мин.)', 10);
            ...
            if (adultAge && noSpecific) addC('🔞 по цензу ' + age + '+ вероятны: нагота, мат, насилие', 14);
            ...
            if (!cons.length) cons.push((blG.length || blA.length || blD.length) ? '✅ под ваши фильтры ничего не попало' : '✅ минусов не нашли');
    ...
    function buildReadyInner(a) {
        ...
        return '' +
            ...
            '</div>' +
            quote +
            '<div class="sw-target-audience"><div class="sw-title target">🎯 Кому понравиться</div><div class="sw-aud-text">' + esc(a.audience) + '</div></div>' +
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v22.3 (syntax fix + living columns).');
    }
