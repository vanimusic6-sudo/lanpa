(function () {
    ...
    /* ===== СТИЛИ (свайп по всей странице, вердикт влезает) ===== */
    function injectCSS() {
        ...
        s.innerHTML =
            ...
            '.sw-quote::before{content:"\201C";position:absolute;left:13px;top:0;font-size:3.3em;line-height:1;color:rgba(255,255,255,.18);font-family:Georgia,serif;font-weight:700}' +
    ...
    // ГЕНЕРАТОР АУДИТОРИИ ИЗ PROS/CONS (живые фразы, не шаблоны)
    function buildAudience(pros, cons, genres, rating, familyOK) {
        var hasCult = pros.some(function(p){ return p.indexOf('культовый') >= 0; });
        var hasHype = pros.some(function(p){ return p.indexOf('свежий хайп') >= 0; });
        var hasGem = pros.some(function(p){ return p.indexOf('скрытая жемчужина') >= 0; });
        var hasAction = pros.some(function(p){ return p.indexOf('драйв') >= 0 || p.indexOf('экшен') >= 0; });
        var hasMusic = pros.some(function(p){ return p.indexOf('музыкальная') >= 0; });
        var hasDoc = pros.some(function(p){ return p.indexOf('познавательно') >= 0; });
        var hasMixed = pros.some(function(p){ return p.indexOf('полярные') >= 0; }) || cons.some(function(c){ return c.indexOf('разочарована') >= 0; });
        var hasViolence = cons.some(function(c){ return c.indexOf('жестокие') >= 0; });
        var hasDrugs = cons.some(function(c){ return c.indexOf('наркотиков') >= 0; });
        var hasNudity = cons.some(function(c){ return c.indexOf('нагота') >= 0; });
        ...
        else if (hasAction) parts.push('ищет драйв без пауз');
        else if (hasThriller) parts.push('любит пощекотать нервы');
        else if (hasComedy) parts.push('ищет повод посмеяться');
        else if (hasDrama && rating >= 7) parts.push('ценит сильные характеры');
        ...
        else if (hasMixed) parts.push('любит спорное кино');

        // Уточнение
        if (familyOK) return 'Семьям для спокойного совместного вечера';
        if (parts.length === 0) return 'Зрителям без жёстких предпочтений';

        var base = 'Тем, кто ' + parts[0];

        // Предупреждение
        if (hasViolence && hasDrugs) base += ' и готов к жёстким сценам с наркотиками';
        else if (hasViolence) base += ' и готов к жёстким сценам';
        else if (hasDrugs) base += ' и готов к темам зависимости';
        else if (hasNudity) base += ' и не против откровенных сцен';
    ...
    function analyze(movie) {
        return Promise.all([ loadCredits(movie), loadMeta(movie) ]).then(function(arr) {
            ...
            var fDrugs    = inAnyText(ovBoth, /метамфетамин|варк|нарко|кокаин|героин|марихуан|каннабис|опиум|амфетамин/) ||
                            inAnyText(ovBoth, /\bmeth\b|cocaine|coke|heroin|marijuan|cannabis|substance|quaalude|\blsd\b|ecstasy|opium|overdose|\bdealer\b|cartel|crack|drug use|drug deal|drug addict/) ||
                            hasKw(ctx, /drug|narcotic|addiction|meth|cocaine|coke|heroin|marijuan|substance|quaalude|\blsd\b|ecstasy|opium|overdose|dealer|cartel|crack/);
            ...
            var fSex      = inAnyText(ovBoth, /\bsex\b|эротик|откровен|оргазм|проститутк/) ||
                            inAnyText(ovBoth, /\borgy\b|threesome|one.?night.?stand|hooker|prostitut|seduction|affair|infidelity|erotic|explicit sex|orgasm|sex scene/) ||
            ...
            var fProfanity= inAnyText(ovBoth, /мат|нецензур|ругательств|брани/) ||
                            inAnyText(ovBoth, /profanity|\bf word\b|strong language|vulgarity|cursing|bad language|swearing|cuss/) ||
            ...
            var fGamb     = inAnyText(ovBoth, /casino|gambl|казино|ставк|\bbet\b|рулетк|покер/) || hasKw(ctx, /casino|gambl|betting|poker/);
            ...
            if (rating >= 7.8 && votes >= 100 && votes < 1500) addP('🔎 скрытая жемчужина — высокий рейтинг узким кругом', 16);
            if (rating >= 8.0 && votes >= 2000 && yr > 0 && yr <= now - 3) addP('🏛 культовый статус — выдержал проверку временем', 18);
            if (yr >= now - 1 && votes >= 200) addP('🔥 свежий хайп — сейчас обсуждают', 10);
            ...
            if (inAnyText(ovBoth, /soundtrack|music|composer|score|музык|композитор/) || hasKw(ctx, /music|soundtrack|composer/)) addP('🎵 сильная музыкальная часть', 8);
            ...
            if (fNudity) addC('🫣 откровенная нагота', 14);
            ...
            if (adultAge && noSpecific) {
                addC('🔞 по цензу ' + age + '+ вероятны: нагота, мат, насилие', 14);
            }
            ...
            if (!cons.length) cons.push((blG.length || blA.length || blD.length) ? '✅ под твои фильтры ничего не попало' : '✅ стоп-факторов не нашли — зелёный свет');
    ...
    /* ===== BACK / КОНТРОЛЛЕР (увеличенный шаг скролла) ===== */
    ...
    function registerController() {
        Lampa.Controller.add('should_watch_modal', {
            ...
            up: function() { var h = window._sw_currentModalHtml; if (h && h[0] && h[0].scrollTop > 0) h[0].scrollBy({ top: -200, behavior: 'smooth' }); },
            down: function() { var h = window._sw_currentModalHtml; if (h && h[0]) { var max = h[0].scrollHeight - h[0].clientHeight; if (h[0].scrollTop < max) h[0].scrollBy({ top: 200, behavior: 'smooth' }); } },
    ...
    function buildReadyInner(a) {
        ...
        return '' +
            ...
            '</div>' +
            quote +
            '<div class="sw-target-audience"><div class="sw-title target">🎯 Кому зайдёт</div><div class="sw-aud-text">' + esc(a.audience) + '</div></div>' +
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v22.2 (swipe fix, scroll fix, dynamic audience).');
    }
