(function () {
    ...
    /* ===== СКРОЛЛ: родной контейнер модалки Lampa ===== */
    function findModalWrap(node) {
        var el = node;
        while (el && el.nodeType === 1 && el !== document.body) {
            var cn = typeof el.className === 'string' ? el.className : '';
            if (/modal(-|__)content/.test(cn)) return el;
            el = el.parentNode;
        }
        return null;
    }
    ...
    function getScrollContainer() {
        if (window._sw_wrap && window._sw_wrap.scrollHeight > window._sw_wrap.clientHeight + 4) return window._sw_wrap;
    ...
    function analyze(movie) {
        return Promise.all([loadCredits(movie), loadMeta(movie), Promise.resolve(lampaLocal(movie))]).then(function(arr){
            ...
            var dGamb = dim(/gambl|casino|poker|betting|bookmaker|lottery|roulette|slot machine|blackjack/, /азарт|казино|ставк|покер|рулетк|лотере|букмекер|автомат/);
            ...
            if (cGamb >= 40) add('tmdb','con','🎰 Затрагивается тема азартных игр', 10, 'gamb');
    ...
    function showModal(movie) {
        try {
            ...
            /* каноничный скролл: используем родной контейнер модалки Lampa.
               На ТВ Lampa ставит ему overflow:hidden — включаем auto, чтобы
               листалось стрелками через наш контроллер; на сенсоре это же
               даёт нативную прокрутку пальцем. */
            try {
                var wrap = findModalWrap(html[0]);
                if (wrap) {
                    wrap.style.overflowY = 'auto';
                    wrap.style.webkitOverflowScrolling = 'touch';
                    wrap.scrollTop = 0;
                    window._sw_wrap = wrap;
                }
            } catch(e) {}
            ...
            analyze(movie).then(function(a){
                ...
                setTimeout(function(){
                    ...
                    if (window._sw_blocknav) { try { if (window._sw_wrap) window._sw_wrap.scrollTop = 0; } catch(e){} highlightVisible(); }
                }, 100);
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v24.0 (native modal scroll on TV + touch)');
    }
