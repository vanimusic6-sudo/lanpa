(function () {
    ...
    function analyze(movie) {
        return Promise.all([ loadCredits(movie), loadMeta(movie) ]).then(function(arr) {
            ...
            if (rating > 0 && rating < cfg.min_rating && votes >= 100) addC('📉 оценки ниже порога (' + rating.toFixed(1) + ')', 22);
            ...
            if (yr >= now - 1 && votes >= 200) addP('🔥 сейчас обсуждают', 10);
            ...
            if (runtime > 0 && runtime <= 120) addP('🕐 хранометраж (' + runtime + ' мин.)', 6);
