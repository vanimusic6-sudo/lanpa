(function () {
    ...
    /* ==========================================================================
       НАСТРОЙКИ (Groq — бесплатно, без VPN)
       ========================================================================== */
    ...
    function initSettings() {
        ...
        [
            { name: 'api_key', type: 'input', title: 'Groq API-ключ (бесплатно)', description: 'Получить за 2 мин: console.groq.com/keys (начинается с gsk_...)', default: '' },
    ...
    /* ==========================================================================
       GROQ API (бесплатно, Llama 3.3 70B, работает из РФ)
       ========================================================================== */

    function analyzeWithGroq(movie) {
        ...
        var systemPrompt = 'Ты — эксперт-кинокритик. Анализируй фильмы конкретно, без воды. Используй эмодзи в начале каждого пункта. Отвечай ТОЛЬКО валидным JSON без markdown.';

        var userPrompt = 'Проанализируй "' + title + '" (' + year + ').\n' +
            'Жанры: ' + genres + '\n' +
            'Синопсис: ' + overview + '\n' +
            'Рейтинг: ' + rating + '/10 (' + votes + ' голосов)\n\n' +
            'Верни JSON:\n' +
            '{\n' +
            '  "pros": ["⭐ высокий рейтинг (8.7)", "🎬 режиссёр: имя"],\n' +
            '  "cons": ["🔪 жестокие сцены", "⌛ высокий хронометраж"],\n' +
            '  "audience": "Любителям экшена и драмы."\n' +
            '}\n\n' +
            'ПРАВИЛА:\n' +
            '- pros: 3-7 пунктов с эмодзи (⭐🎵💥🤱🦫🎥🕐🏳️🔎🔥🎬✍️🎭🎞🇷🇺🌍🔗🏷)\n' +
            '- cons: 0-7 пунктов с эмодзи (💋🚬🎰🔪⌛💉📺🚩⛔). Проверяй: наркотики, насилие, курение, алкоголь, нагота, жестокость, низкое качество\n' +
            '- audience: одно предложение, начинай с "Любителям"\n' +
            '- Указывай КОНКРЕТИКУ: имена актёров, режиссёров, точные цифры\n' +
            '- Для "Рика и Морти" ОБЯЗАТЕЛЬНО укажи: наркотики, насилие, курение, не для детей\n' +
            '- Для "Во все тяжкие" ОБЯЗАТЕЛЬНО: наркотики (метамфетамин), насилие, криминал\n' +
            '- НЕ пиши общих фраз типа "интересный сюжет", "хорошая игра актёров"\n' +
            '- Пиши на русском';
                ...
                .then(function(data) {
                    try {
                        if (data && data.choices && data.choices[0] && data.choices[0].message) {
                            ...
                            console.error('[SW] Groq response error:', data);
                            res(null);
                        }
    ...
    /* ==========================================================================
       АНАЛИЗАТОР v12.0 (Groq + fallback на теги)
       ========================================================================== */
    ...
    function analyze(movie) {
        ...
        return Promise.all([ loadCredits(movie), loadExtra(movie), useAI ? analyzeWithGroq(movie) : Promise.resolve(null) ]).then(function(arr) {
            ...
            if (aiResult && aiResult.pros && aiResult.cons) {
                ...
                var audience = aiResult.audience && typeof aiResult.audience === 'string' ? aiResult.audience : 'Любителям кино.';
    ...
    function showModal(movie) {
        ...
        analyze(movie).then(function(a) {
            ...
            var badge = '';
            if (a.mode === 'AI') {
                badge = '<span class="sw-mode-badge ai"><span class="sw-mode-dot active"></span>GROQ</span>';
            } else if (a.mode === 'AI_ERROR') {
                badge = '<span class="sw-mode-badge error"><span class="sw-mode-dot inactive"></span>AI (ошибка)</span>';
            } else {
                badge = '<span class="sw-mode-badge tags"><span class="sw-mode-dot inactive"></span>TAGS</span>';
            }
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v12.0 initialized (Groq FREE — Llama 3.3 70B).');
    }
