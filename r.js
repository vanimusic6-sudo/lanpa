(function () {
    ...
    /* ===== СПИСОК ИЗВЕСТНЫХ ДЕТСКИХ ФИЛЬМОВ (для корректировки оценки) ===== */
    var KNOWN_FAMILY_MOVIES = [
        ...
        'геркулес', 'hercules',
        'тараканы', 'a bug\'s life',
        'монстры на каникулах', 'monsters inc',
        ...
        'moana',
        'zootopia', 'зоотопия',
        'coco', 'там, где нас нет',
        'soul', 'душа',
        'luca', 'лука',
        'onward', 'вперёд',
        'encanto', 'энканто',
        'turning red', 'краснеет',
        'lightyear', 'лайтйир',
        'elemental', 'элементарно',
        'spider-verse', 'человек-паук: через вселенные',
        'mitchells vs the machines', 'семейка митчелов против машин',
        'shrek', 'шрек',
    ...
    function isKnownFamilyMovie(title, originalTitle) {
        ...
        return KNOWN_FAMILY_MOVIES.some(function(movie) {
            return checkTitle.indexOf(movie) >= 0 || checkOriginal.indexOf(movie) >= 0;
        });
    }
    ...
    function initNavPoints(html) {
        try {
            ...
            // Собираем ключевые точки для навигации: верх, середина, низ
            window._sw_navPoints = [];
            
            // 1. Верхний блок (вердикт)
            ...
            // 3. Нижний блок (кнопка)
            var bottomPoint = html.find('.sw-dice-btn');
            if (bottomPoint.length) {
                bottomPoint.addClass('sw-nav-point');
                window._sw_navPoints.push(bottomPoint);
            }
            
            // Если не нашли, используем все focusable элементы
    ...
    function scrollToNavPoint(index) {
        try {
            ...
            if (point && point.length && container && container.length) {
                ...
                container.animate({
                    scrollTop: point[0].offsetTop - 20
                }, 200, function() {
                    // Ставим фокус на точку
                    try {
    ...
    function scrollToElement(el, container) {
        try {
            ...
            else if (elTop + el[0].offsetHeight > containerBottom - 20) {
    ...
    function buildAudience(pros, cons, genres, rating, familyOK, age, isAnim, ctx, movieTitle, movieOriginalTitle) {
        ...
        var hasAnimation = hasGenre(genres, /animation|мульт|анимац/i);
        ...
        // Положительные характеристики - ЕСТЕСТВЕННЫЕ ФОРМУЛИРОВКИ
        ...
        if (isAnim && !familyOK && age !== null && age >= 16) {
            base = 'Фанатам анимации, которые не против ' + warnings.join(', ') || 'взрослого контента';
        }
        
        // Если это известный детский фильм, но что-то пошло не так
    ...
    function analyze(movie) {
        return Promise.all([ loadCredits(movie), loadMeta(movie) ]).then(function(arr) {
            ...
            // Считаем семейным если:
            // 1. Нет взрослого контента
            // 2. Рейтинг >= 5 (понизил порог для детских фильмов)
            // 3. Либо есть явные детские маркеры (age <= 12 или kidsKw или известный детский фильм)
            var familyOK = !anyAdult && rating >= 5 && ((age !== null && age <= 12) || kidsKw || isKnownFamily);
            
            // Дополнительная проверка для анимации
            if (isAnim && !kidsKw && !isKnownFamily && (age === null || age >= 13)) {
                // Если анимация без явных детских маркеров и с возрастным рейтингом >= 13, не считаем семейной
                // Исключение: если это известный детский фильм
                if (!isKnownFamily) {
                    familyOK = false;
                }
            }
            
            // Если это известный детский фильм, но что-то пошло не так с возрастом
            ...
            // === ПЛЮСЫ (ЕСТЕСТВЕННЫЕ ФОРМУЛИРОВКИ) ===
            ...
            // === МИНУСЫ (ЕСТЕСТВЕННЫЕ ФОРМУЛИРОВКИ) ===
            ...
            if (isAnim && !familyOK && !isKnownFamily) addC('🎭 не детский мультфильм', 16);
            
            // Дополнительные предупреждения
            var adultAge = (age !== null && age >= 16);
            var noSpecific = !fNudity && !fSex && !fProfanity && !fViol && !fDrugs && !fAlcohol && !fSuicide;
            if (adultAge && noSpecific) addC('🔞 возможно presence взрослого контента (' + age + '+)', 16);
    ...
    /* ===== BACK / КОНТРОЛЛЕР (TV: прыжки по точкам, смартфон: плавный скролл) ===== */
            ...
            Lampa.Controller.add('should_watch_modal', {
                toggle: function() {
                    ...
                    if (window._sw_blocknav) {
                        ...
                        swSetFocus(0);
                        // Инициализируем точки навигации
                        initNavPoints(h);
                    }
                },
                up: function() { 
                    if (window._sw_blocknav) {
                        // Для TV: прыгаем между ключевыми точками
                        if (window._sw_navPoints && window._sw_navPoints.length > 0) {
                            ...
                            // Если точек нет, используем старую логику
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v24.0 (TV navigation points + improved family detection + natural phrases).');
    }
