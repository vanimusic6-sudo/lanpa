(function () {
    ...
    function analyze(movie) {
        return Promise.all([loadCredits(movie), loadMeta(movie), Promise.resolve(lampaLocal(movie)), loadKp(movie)]).then(function(arr){
            ...
            var whoGen = domRevs.length ? 'комментаторов' : 'зрителей';
            var ctx = { kw: meta.kw };
            var M = 150, C = 6.1;
            ...
            if (rt.total >= 2 && rt.tone === 'pos') add('reviews','pro','💬 ' + who + ' в восторге: ' + rt.pos + ' из ' + rt.total + ' отзывов положительные', 20, 'rev');
            else if (rt.total >= 2 && rt.tone === 'neg') add('reviews','con','💬 Может разочаровать: ' + rt.neg + ' из ' + rt.total + ' отзывов отрицательные (' + whoGen + ')', 22, 'rev');
            else if (rt.total >= 2 && rt.tone === 'mix') { add('reviews','pro','💬 Отзывы полярные: ' + rt.pos + ' из ' + rt.total + ' положительных', 6, 'rev'); add('reviews','con','💬 Часть ' + whoGen + ' осталась недовольна', 8); }
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v22.0 (commenters label, no quote block)');
    }
