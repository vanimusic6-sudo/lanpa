(function () {
    ...
    /* ===== СТИЛИ (без слогана и подписей шкалы; живой скролл; display-шрифт на заголовках) ===== */
    function injectCSS() {
    ...
    function analyze(movie) {
        return Promise.all([ loadCredits(movie), loadMeta(movie) ]).then(function(arr) {
            ...
            var anyAdult = fDrugs || fViol || fSmoke || fSex || fHate || fAdultAnim || !!movie.adult || (age !== null && age >= 16);

            // СЕМЕЙНЫЙ — только при явном подтверждении, жанр animation сам по себе НЕ даёт семейный
            ...
            // Интерпретации рейтинга/славы (pop больше не участвует в «тихом шедевре»)
            ...
            // Честный сигнал для анимации, не прошедшей семейный фильтр (закрывает Рика и Морти без тегов)
            ...
            // Живая заглушка вместо «явных стоп-факторов не найдено»
            ...
            // Кому зайдёт — теги в одном падеже, без кривой вводной
            var aud = [];
        ...
        Lampa.Controller.add('should_watch_modal', {
            toggle: function() {
                ...
                if (h) {
                    ...
                    var b = h.find('#sw-dice-btn'); if (b.length) Lampa.Controller.collectionFocus(b);
                }
            },
    ...
    /* ===== РЕНДЕР (без слогана, без подписей шкалы; аудитория — теги) ===== */
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v21.0 (family-fix, cult-status, aud-tags, scroll).');
    }
