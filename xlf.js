(function () {
    ...
    /* ===== БЛОЧНАЯ НАВИГАЦИЯ С ГАРАНТИРОВАННОЙ ДОКРУТКОЙ ===== */
    function swSetFocus(i) {
        try {
            ...
            // Двойная гарантия докрутки: сначала scrollIntoView, потом коррекция scrollTop
            try { el[0].scrollIntoView({ block: 'center', behavior: 'smooth' }); } catch(e) {}
            
            // Принудительная докрутка через 100мс (когда DOM точно отрисовался)
            setTimeout(function() {
                try {
                    var container = h[0];
    ...
    function buildAudience(pros, cons, genres, rating, familyOK) {
        ...
        var hasHype = pros.some(function(p){ return p.indexOf('хайп') >= 0; });
        var hasGem = pros.some(function(p){ return p.indexOf('жемчужина') >= 0; });
    ...
    function registerController() {
        try {
            Lampa.Controller.add('should_watch_modal', {
                ...
                up: function() { if (window._sw_blocknav) swSetFocus((window._sw_focusIdx || 0) - 1); },
                down: function() { if (window._sw_blocknav) swSetFocus((window._sw_focusIdx || 0) + 1); },
                left: function() {}, right: function() {},
                back: function() {
                    window._sw_rolling = false; window._sw_currentModalHtml = null; window._sw_focusIdx = 0; clearLoader();
            ...
            try {
                ...
                setTimeout(function() {
                    try {
                        ...
                        try { btn[0].scrollIntoView({ block: 'center' }); } catch(_) {}
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v22.6 (guaranteed scroll follow).');
    }
