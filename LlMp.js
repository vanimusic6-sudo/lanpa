(function () {
    ...
    function injectCSS() {
        try {
            ...
            s.innerHTML =
                ...
                '/* Fix for scrollable container */ .sw-modal-content { scroll-snap-type: y mandatory; } .sw-focusable { scroll-snap-align: start; }';
    ...
    function scrollToElement(el, container) {
        try {
            ...
            else if (elBottom > containerBottom - 20) {
                container.animate({ scrollTop: elTop - containerHeight + el[0].offsetHeight + 20 }, 200);
            }
    ...
    function mapUSRating(s) { 
        return { 
            ...
            'MA':17, '18':18, '16':16, '12':12, '12A':12, '15':15 
    ...
    function reviewTone(reviews) {
        ...
        var posRe = /шедевр|великолепн|потрясающ|восхитит|блестящ|лучш|мощн|гениальн|masterpiece|brilliant|amazing|great|best|loved|perfect|outstanding|precisely|flawless|must-watch/i;
        var negRe = /скучн|ужасн|провал|разочаров|слаб|затян|бессмысл|плох|boring|bad|worst|terrible|awful|disappoint|waste|dull|pointless|ridiculous/i;
    ...
    function buildAudience(pros, cons, genres, rating, familyOK, age, isAnim, ctx) {
        var hasCult = pros.some(function(p){ return /культ|классик/i.test(p.toLowerCase()); });
        var hasHype = pros.some(function(p){ return /хайп|популярн|обсуждаем/i.test(p.toLowerCase()); });
        var hasGem = pros.some(function(p){ return /жемчужин|скрыт|недооцен/i.test(p.toLowerCase()); });
        var hasAction = hasGenre(genres, /action|боевик|экшен/i) || pros.some(function(p){ return /драйв|экшен/i.test(p.toLowerCase()); });
        var hasMusic = pros.some(function(p){ return /музык|саундтрек|композитор/i.test(p.toLowerCase()); });
        var hasDoc = hasGenre(genres, /documentary|документ/i) || pros.some(function(p){ return /познавательн|обучающ/i.test(p.toLowerCase()); });
        ...
        var hasCrime = hasGenre(genres, /crime|криминал/i);
        var hasAdventure = hasGenre(genres, /adventure|приключен/i);
        var hasRomance = hasGenre(genres, /romance|мелодрама|любовн/i);
        
        var hasViolence = cons.some(function(c){ return /жесток|насили|кров|убийств/i.test(c.toLowerCase()); }) || hasKw(ctx, /violenc|gore|murder|blood/);
        var hasDrugs = cons.some(function(c){ return /нарко|метамфетамин|кокаин/i.test(c.toLowerCase()); }) || hasKw(ctx, /drug|narcotic|meth|cocaine/);
        ...
        if (hasCult && rating >= 8) parts.push('ценит проверенную классику');
        else if (hasHype) parts.push('хочет быть в курсе обсуждаемого');
        else if (hasGem) parts.push('любит открывать недооценённое');
        else if (hasAction) parts.push('ищет драйв и острые ощущения');
        else if (hasThriller) parts.push('любит пощекотать нервы');
        else if (hasComedy) parts.push('ищет повод посмеяться');
        else if (hasDrama && rating >= 7) parts.push('ценит глубокие истории');
        else if (hasSciFi) parts.push('хочет уйти в другой мир');
        else if (hasDoc) parts.push('любит узнавать новое');
        else if (hasMusic) parts.push('ценит сильную музыку');
        ...
        if (familyOK) return 'Семьям для спокойного совместного вечера';
        
        var base = 'Тем, кто ' + parts[0];
        
        // Добавляем предупреждения о контенте
        var warnings = [];
        if (hasViolence && hasDrugs) warnings.push('жёстким сценам с наркотиками');
        else if (hasViolence) warnings.push('жёстким сценам');
        if (hasDrugs && !hasViolence) warnings.push('темам зависимости');
        if (hasNudity) warnings.push('откровенным сценам');
        if (hasStrongLang) warnings.push('нецензурной лексике');
        if (age !== null && age >= 16) warnings.push('взрослому контенту (' + age + '+)');
        
        if (warnings.length > 0) {
            base += ' и готов к ' + warnings.join(', ');
        }
        
        // Для анимации
        if (isAnim && !familyOK && age !== null && age >= 16) {
            base = 'Фанатам взрослой анимации, которые не против ' + warnings.join(', ') || 'взрослого контента';
        }
        
        return base;
    }

    /* ===== АНАЛИЗ (ИСПРАВЛЕННАЯ ЛОГИКА) ===== */
    function analyze(movie) {
        return Promise.all([ loadCredits(movie), loadMeta(movie) ]).then(function(arr) {
            ...
            var kidsKw = hasKw(ctx, /for kids|children|kids|family-friendly|kids tv|детям|семейн|для детей/i);

            // Улучшенное определение взрослого контента
            ...
            // Общий флаг взрослого контента
            var anyAdult  = fDrugs || fNudity || fSex || fViol || fSmoke || fAlcohol || fProfanity || fHate || fGamb || fAdultAnim || fSuicide || !!movie.adult || (age !== null && age >= 16);
            
            // Улучшенное определение familyOK
            // Считаем семейным только если:
            // 1. Нет взрослого контента
            // 2. Рейтинг >= 6
            // 3. Либо есть явные детские маркеры (age <= 12 или kidsKw)
            var familyOK = !anyAdult && rating >= 6 && ((age !== null && age <= 12) || kidsKw);
            
            // Дополнительная проверка для анимации
            if (isAnim && !kidsKw && (age === null || age >= 13)) {
                // Если анимация без явных детских маркеров и с возрастным рейтингом >= 13, не считаем семейной
                familyOK = false;
            }
            ...
            else if (rating >= 8.0 && votes >= 3000) addP('⭐ признание зрителей и критиков', 30);
            else if (rating >= cfg.min_rating && votes >= 500) addP('⭐ оценки стабильно выше среднего', 20);
            else if (rating >= cfg.min_rating && votes >= 100) addP('⭐ оценки выше твоего порога', 18);
            
            if (rating >= 7.8 && votes >= 100 && votes < 1500) addP('🔎 скрытая жемчужина — высокий рейтинг в узком кругу', 18);
            if (rating >= 8.0 && votes >= 2000 && yr > 0 && yr <= now - 3) addP('🏛 культовый статус', 20);
            if (yr >= now - 1 && votes >= 200) addP('🔥 на хайпе — все обсуждают', 12);
            if (yr >= now && yr <= now) addP('🆕 свежая новинка', 8);
            if (votes > 0 && votes < 30) addC('❓ оценок слишком мало — вердикт осторожный', 12);
            if (votes === 0) addC('⚠️ нет оценок — данных недостаточно', 15);

            // Отзывы
            if (rt.tone === 'pos') addP('💬 зрители в восторге — редкий восторг в отзывах', 22);
            else if (rt.tone === 'neg') addC('💬 в отзывах ругают сюжет и темп', 25);
            else if (rt.tone === 'mix') { 
                addP('💬 отзывы полярные — не оставляет равнодушным', 8); 
                addC('💬 часть зрителей разочарована — на любителя', 10); 
            }

            // Качество
            if (q && !/CAM|TS|HDCAM|SCR|WORKPRINT|TELESYNC|HDRIP|TELECINE/i.test(q)) addP('🎥 отличное качество изображения (' + (q || 'HD') + ')', 10);
            if (q && /4K|UHD|2160p/i.test(q)) addP('🎥 потрясающее 4K-качество', 12);
            if (runtime > 0 && runtime <= 90) addP('🕐 можно посмотреть за вечер (' + runtime + ' мин.)', 8);
            if (runtime > 90 && runtime <= 120) addP('🕐 удобный формат для просмотра (' + runtime + ' мин.)', 6);
            
            // Контент
            if (familyOK) addP('👨‍👩‍👧‍👦 безопасно для семейного просмотра', 16);
            if (hasGenre(genres, /documentary|документ/i)) addP('🦉 познавательно и интересно', 10);
            if (inAnyText(ovBoth, /soundtrack|music|composer|score|музык|композитор|саундтрек/i) || hasKw(ctx, /music|soundtrack|composer|score/)) addP('🎵 запоминающаяся музыкальная часть', 10);
            if (hasGenre(genres, /action|боевик|экшен/i)) addP('💥 яркий экшен и драйв', 10);
            if (meta.hasTrailer) addP('▶ есть трейлер — глянь 2 минуты перед просмотром', 6);
            if (hasGenre(genres, /comedy|комедия/i)) addP('😂 гарантированно поднимет настроение', 8);
            if (hasGenre(genres, /adventure|приключения/i)) addP('🌍 увлекательные приключения', 8);
            if (hasGenre(genres, /sci-fi|фантастика|fantasy|фэнтези/i)) addP('🚀 уносит в другой мир', 8);
            if (hasGenre(genres, /drama|драма/i) && rating >= 7.5) addP('🎭 сильная актёрская игра', 8);
            
            // === МИНУСЫ ===
            if (rating > 0 && rating < cfg.min_rating && votes >= 100) addC('📉 оценки ниже твоего порога (' + rating.toFixed(1) + ')', 25);
            ...
            if (fNudity) addC('🫣 откровенные сцены с наготой', 16);
            if (fSex) addC('💋 сексуальные сцены', 16);
            if (fDrugs) addC('💉 тематика наркотиков', 18);
            if (fViol) addC('🔪 жестокие и кровавые сцены', 18);
            if (fSmoke) addC('🚬 курение в кадре', 8);
            if (fAlcohol) addC('🍺 алкоголь и пьянство', 10);
            if (fProfanity) addC('🤬 обилие нецензурной лексики', 10);
            if (fHate) addC('🚩 мотивы ненависти или расизма', 20);
            if (fGamb) addC('🎰 тема азартных игр и ставок', 12);
            if (fSuicide) addC('⚠️ тематика суицида', 20);
            if (runtime > 180) addC('⌛ сложно посмотреть за раз (' + runtime + ' мин.)', 12);
            if (runtime > 150 && runtime <= 180) addC('⌛ довольно длительный (' + runtime + ' мин.)', 8);
            if (/CAM|TS|HDCAM|HDRIP|TELECINE|SCR|WORKPRINT|TELESYNC/i.test(q || '')) addC('📺 экранка — плохое качество звука и изображения', 28);
            if (age !== null && age >= 18) addC('🔞 ценз ' + age + '+ — только для взрослых', 15);
            else if (age !== null && age >= 16) addC('🔞 ценз ' + age + '+ — не для детей', 14);
            else if (age !== null && age >= 12) addC('🔞 ценз ' + age + '+ — рекомендуется с родителями', 10);
            if (isAnim && !familyOK) addC('🎭 не детский мультфильм — возможен взрослый юмор и темы', 16);
            
            // Дополнительные предупреждения для взрослого контента
            var adultAge = (age !== null && age >= 16);
            var noSpecific = !fNudity && !fSex && !fProfanity && !fViol && !fDrugs && !fAlcohol && !fSuicide;
            if (adultAge && noSpecific) addC('🔞 по цензу ' + age + '+ — возможны: нагота, мат, насилие', 16);
            ...
            var reason = '';
            if (vClass === 'yes') {
                reason = 'Перевесили аргументы «за»' + (topP ? ' — ' + strip(topP.t) : '') + '.';
                if (score >= 50) reason = 'Определённо стоит посмотреть' + (topP ? ' — ' + strip(topP.t) : '') + '.';
            }
            else if (vClass === 'no') {
                reason = 'Решающий аргумент «против»' + (topC ? ' — ' + strip(topC.t) : '') + '.';
                if (score <= -50) reason = 'Лучше пропустить' + (topC ? ' — ' + strip(topC.t) : '') + '.';
            }
            else {
                reason = 'Аргументы в балансе' + (topP && topC ? ': за «' + strip(topP.t) + '», против «' + strip(topC.t) + '».' : '.') + ' Тут решает вкус.';
            }
            ...
            if (!cons.length) cons.push((blG.length || blA.length || blD.length) ? '✅ под ваши фильтры ничего не попало' : '✅ явных минусов не нашли');
                ...
                up: function() { 
                    if (window._sw_blocknav) {
                        ...
                        if (container && container.scrollTop() > 50) {
                            container.animate({ scrollTop: container.scrollTop() - 40 }, 100);
                        } else {
    ...
    function buildReadyInner(a) {
        ...
        return '' +
            '<div class="sw-dossier sw-focusable" tabindex="0">' + badge +
            ...
            '</div>' +
            quote +
            '<div class="sw-target-audience sw-focusable" tabindex="0"><div class="sw-title target">🎯 Кому понравится</div><div class="sw-aud-text">' + esc(a.audience) + '</div></div>' +
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v23.0 (Исправлена навигация, оценка возраста и тексты).');
    }
