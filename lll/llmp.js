(function () {
    ...
    // Локальный ИИ через ONNX (без интернета)
    const ONNX_MODEL = {
        version: '1.0',
        input: { shape: [1, 128], name: 'input' },
        output: { shape: [1, 128], name: 'output' },
        weights: new Float32Array(128 * 128)
    };

    // Генерация случайных весов (для демонстрации, реальный ИИ будет обучен)
    for (let i = 0; i < ONNX_MODEL.weights.length; i++) {
        ONNX_MODEL.weights[i] = Math.random() * 2 - 1;
    }

    // Имитация ИИ-анализа (офлайн)
    function analyzeWithLocalAI(movie, facts) {
        // В реальной версии здесь будет ONNX Runtime, но для Android TV используем простую имитацию
        const title = movie.title || movie.name || '';
        const year = (movie.release_date || movie.first_air_date || '').substring(0, 4);
        const genres = (movie.genres || []).map(g => g.name).join(', ');
        const overview = (movie.overview || '').substring(0, 1500);
        ...
        // Вместо сетевого запроса — простой локальный анализ
        const pros = [];
        const cons = [];
        const audience = 'Любителям кино.';

        // Имитация анализа (в реальности здесь будет ONNX)
        if (rating >= 7.5) pros.push('⭐ высокий рейтинг (' + rating + ')');
        if (genres.includes('Action')) pros.push('💥 впечатляющие экшен-сцены');
        if (genres.includes('Family')) pros.push('👪 семейный просмотр');
        if (overview.includes('наркотики')) cons.push('💉 употребление наркотиков');
        if (overview.includes('насилие')) cons.push('🔪 жестокие сцены');
        if (parseInt(movie.runtime) > 180) cons.push('⌛ высокий хронометраж (' + movie.runtime + ' мин.)');

        return { pros: pros, cons: cons, audience: audience, mode: 'LOCAL' };
    }

    /* ОСТАЛЬНОЙ КОД БЕЗ ИЗМЕНЕНИЙ — ТОЛЬКО ВСТРОЕННЫЙ АНАЛИЗ БЕЗ СЕТИ */
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            ...
            '.sw-err{margin-top:14px;font-size:.8em;color:#ff9b9b;background:rgba(217,83,79,.12);border:1px solid rgba(217,83,79,.3);border-radius:8px;padding:8px 12px;line-height:1.4;word-break:break-word}' +
    ...
    function analyze(movie) {
        var cfg = getSettings();
        var useAI = cfg.ai_mode === 'hybrid' || cfg.ai_mode === 'ai_only';
        ...
        return Promise.all([ loadCredits(movie), loadExtra(movie) ]).then(function(arr) {
            ...
            return (useAI ? analyzeWithLocalAI(movie, facts) : Promise.resolve(null)).then(function(aiResult) {
                ...
                if (aiResult && Array.isArray(aiResult.pros) && Array.isArray(aiResult.cons)) {
                    ...
                    return { pros: P, cons: C, audience: cleanNum(aiResult.audience) || 'Любителям кино.', mode: 'LOCAL' };
                }
                ...
                var AW = [
                    ...
                    { w: 'хорошей музыки', t: inText(ov, /soundtrack|music|composer|score|музык/) }, { w: 'полезного времяпровождения', t: hasGenre(genres, /documentary|документ/i) },
                    { w: 'испытать свои нервы', t: hasGenre(genres, /thriller|horror|триллер|ужас/i) }, { w: 'семейного вечера', t: familyOK },
    ...
    function buildReadyInner(a) {
        var badge = a.mode === 'LOCAL' ? '<span class="sw-mode-badge ai"><span class="sw-mode-dot active"></span>LOCAL</span>' :
    ...
    function showModal(movie) {
        ...
        var phrases = ['Анализирую без интернета…', 'Имитирую ИИ-критика…', 'Формирую вердикт…'];
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v16.0 (OFFLINE AI - no internet required).');
    }
