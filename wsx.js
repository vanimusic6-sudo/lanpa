(function () {
    ...
    /* ===== СТИЛИ (досье-шапка, шкала-настроение, выноска-цитата) ===== */
    function injectCSS() {
        if (document.getElementById('sw-plugin-styles')) return;
        var s = document.createElement('style'); s.id = 'sw-plugin-styles';
        s.innerHTML =
            '.sw-modal-content{padding:22px 24px;color:#fff;font-family:sans-serif;max-height:78vh;overflow-y:auto;scroll-behavior:smooth}' +
            ...
            '.sw-loader-text{font-size:1.1em;font-weight:600;min-height:1.4em;transition:opacity .25s;color:#94a3b8;letter-spacing:.01em}' +
            ...
            /* досье-шапка */
            '.sw-dossier{position:relative;padding:22px 24px 24px;border-radius:16px;margin-bottom:24px;overflow:hidden;background:linear-gradient(160deg,rgba(255,255,255,.06),rgba(255,255,255,.015));border:1px solid rgba(255,255,255,.08)}' +
            ...
            '.sw-verdict-word{font-size:2.6em;font-weight:900;letter-spacing:-.02em;line-height:1;margin:0 0 4px;text-transform:uppercase}' +
            '.sw-verdict-word.yes{color:#85c25e}.sw-verdict-word.no{color:#d9534f}.sw-verdict-word.maybe{color:#e0a93b}' +
            '.sw-verdict-reason{font-size:1em;color:#c4ccd6;line-height:1.45;margin:0 0 16px;max-width:60ch}' +
            '.sw-meter{height:10px;border-radius:6px;background:rgba(0,0,0,.35);overflow:hidden;position:relative;box-shadow:inset 0 1px 3px rgba(0,0,0,.4)}' +
            ...
            '.sw-meter-scale{display:flex;justify-content:space-between;font-size:.72em;color:#7c828c;margin-top:6px;letter-spacing:.04em;text-transform:uppercase}' +
            '.sw-tagline{margin-top:16px;font-style:italic;font-size:1.18em;color:#e7ecf3;line-height:1.4;border-left:3px solid rgba(255,255,255,.25);padding-left:14px}' +
            '.sw-mode-badge{position:absolute;top:18px;right:20px;display:inline-flex;align-items:center;gap:6px;font-size:.7em;padding:4px 11px;border-radius:10px;font-weight:700;letter-spacing:.06em;text-transform:uppercase}' +
            ...
            /* колонки аргументов */
            '.sw-columns{display:flex;justify-content:space-between;gap:20px;margin-bottom:22px}' +
            '.sw-col{flex:1;background:rgba(255,255,255,.04);padding:16px 18px;border-radius:12px;border:1px solid rgba(255,255,255,.06)}' +
            '.sw-title{font-size:1.02em;font-weight:800;margin-bottom:14px;text-transform:uppercase;display:flex;align-items:center;gap:8px;letter-spacing:.03em}' +
            ...
            /* выноска-цитата отзыва */
            '.sw-quote{position:relative;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.07);border-radius:14px;padding:20px 22px 18px 52px;margin-bottom:22px}' +
            '.sw-quote::before{content:"“";position:absolute;left:14px;top:2px;font-size:3.4em;line-height:1;color:rgba(255,255,255,.18);font-family:Georgia,serif;font-weight:700}' +
            '.sw-quote-text{font-size:1.04em;line-height:1.55;color:#dfe5ec;font-style:italic}' +
            '.sw-quote-meta{margin-top:10px;font-size:.82em;color:#8b929c;letter-spacing:.02em}' +
            ...
            '.sw-target-audience{background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.06);padding:18px 20px;border-radius:12px;line-height:1.6;color:#d6dce4;font-size:1.04em;margin-bottom:22px}' +
            /* секция решения */
            '.sw-decision{text-align:center;padding:22px 20px;background:rgba(255,255,255,.03);border-radius:14px;border:1px solid rgba(255,255,255,.06)}' +
            '.sw-decision-hint{font-size:.86em;color:#8b929c;margin-bottom:14px;letter-spacing:.01em}' +
            '.sw-dice-btn{background:#eadecd;color:#1a1a1a;font-size:1.25em;font-weight:800;padding:14px 34px;border-radius:30px;display:inline-flex;align-items:center;gap:12px;transition:transform .2s,background .2s,box-shadow .2s;cursor:pointer;outline:none;border:3px solid transparent}' +
            ...
            '.sw-verdict-roll{margin-top:14px;font-size:1.5em;font-weight:900;min-height:38px;text-transform:uppercase;letter-spacing:.01em;transition:color .2s}' +
            ...
            '@media(max-width:640px){.sw-columns{flex-direction:column}.sw-verdict-word{font-size:2.1em}}';
    ...
    /* ===== ДОГРУЗКА: credits + keywords + ценз + отзывы + трейлер (один батч) ===== */
    ...
    // Тональность отзывов по словарю (грубо, но честно помечено)
    function reviewTone(reviews) {
        if (!reviews.length) return { tone: null, pos: 0, neg: 0, sample: null };
        ...
        return { tone: tone, pos: pos, neg: neg, sample: sample };
    }

    /* ===== АНАЛИЗ: аргументы + взвешенный скор ===== */
    function analyze(movie) {
        return Promise.all([ loadCredits(movie), loadMeta(movie) ]).then(function(arr) {
            ...
            var pop = parseFloat(movie.popularity) || 0;
            ...
            // --- АРГУМЕНТЫ (каждый со своим весом для скора) ---
            var P = [], C = []; // {t, w}
            function addP(t, w) { P.push({ t: t, w: w }); }
            function addC(t, w) { C.push({ t: t, w: w }); }

            // Интерпретации рейтинга/хайпа вместо голой цифры
            if (rating >= 8.0 && votes >= 3000) addP('⭐ признание зрителей и критиков — редкий провал', 30);
            else if (rating >= cfg.min_rating && votes >= 100) addP('⭐ оценки стабильно выше среднего', 18);
            if (rating > 0 && rating < cfg.min_rating && votes >= 100) addC('📉 оценки ниже твоего порога (' + rating.toFixed(1) + ')', 22);
            if (rating >= 7.5 && pop < 60 && votes >= 200) addP('🔎 тихий шедевр — высокий рейтинг без шумихи', 16);
            if (yr > 0 && yr < 2005 && rating >= 7.5) addP('🏛 культовый статус — выдержал проверку временем', 14);
            if (yr >= new Date().getFullYear() - 1 && pop > 200) addP('🔥 свежий хайп — сейчас обсуждают все', 10);
            if (votes > 0 && votes < 30) addC('❓ оценок слишком мало, чтобы доверять рейтингу', 8);

            // Отзывы как живой сигнал
            if (rt.tone === 'pos') addP('💬 зрители в отзывах хвалят — редкий восторг', 20);
            else if (rt.tone === 'neg') addC('💬 в отзывах ругают сюжет и темп', 22);
            else if (rt.tone === 'mix') { addP('💬 отзывы полярные — фильм не оставляет равнодушным', 6); addC('💬 часть зрителей разочарована — на любителя', 8); }

            // Люди — только как аргумент через чёрный список (нейтральный список имён убран)
            // Качество / хронометраж / прочее
            if (q && !/CAM|TS|HDCAM|SCR|WORKPRINT/i.test(q)) addP('🎥 картинка в хорошем качестве (' + (q || 'HD') + ')', 8);
            if (runtime > 0 && runtime <= 120) addP('🕐 укладывается в вечер без усталости (' + runtime + ' мин.)', 6);
            if (familyOK) addP('🤱 безопасно для семейного просмотра', 14);
            if (hasGenre(genres, /documentary|документ/i)) addP('🦫 познавательно — узнаешь что-то новое', 8);
            ...
            if (fSex) addC('💋 откровенные сцены', 14);
            if (fSmoke) addC('🚬 курение / алкоголь в кадре', 10);
            ...
            var norm = Math.round((score + 100) / 2); // 0..100 для шкалы
            var confidence = dataRich ? 'high' : 'low';
            var vClass = score >= 30 ? 'yes' : (score <= -30 ? 'no' : 'maybe');
            var vWord = score >= 30 ? 'СТОИТ' : (score <= -30 ? 'НЕ СТОИТ' : 'СПОРНО');

            // Обоснование вердикта — берём самый весомый аргумент с каждой стороны
            var topP = P.slice().sort(function(a,b){ return b.w - a.w; })[0];
            var topC = C.slice().sort(function(a,b){ return b.w - a.w; })[0];
            var reason = '';
            if (vClass === 'yes') reason = 'Перевесили аргументы «за»' + (topP ? ': ' + topP.t.replace(/^[^\s]+\s/, '') : '') + '.';
            else if (vClass === 'no') reason = 'Решающий аргумент «против»' + (topC ? ': ' + topC.t.replace(/^[^\s]+\s/, '') : '') + '.';
            else reason = 'Аргументы в балансе — тут решает вкус' + (topP && topC ? ': за «' + topP.t.replace(/^[^\s]+\s/, '') + '», против «' + topC.t.replace(/^[^\s]+\s/, '') + '».' : '.');
            if (confidence === 'low') reason += ' (сигналов мало — вердикт осторожный)';

            var pros = P.map(function(x){ return x.t; });
            var cons = C.map(function(x){ return x.t; });
            if (!pros.length) pros.push('ℹ️ мало сигналов, чтобы советовать');
            if (!cons.length) cons.push('✅ явных стоп-факторов не найдено');

            // Кому — не тавтология жанров, а по мотивам
            var aud = [];
            if (hasGenre(genres, /thriller|horror|триллер|ужас/i)) aud.push('тем, кто любит пощекотать нервы');
            ...
            if (familyOK) aud.push('для вечера всей семьёй');
            if (hasGenre(genres, /documentary|документ/i)) aud.push('любознательным');
            if (hasGenre(genres, /sci-fi|fantasy|фантастик/i)) aud.push('тем, кто хочет уйти в другой мир');
            if (rt.tone === 'pos') aud.push('тем, кто доверяет восторгу зрителей');
            if (!aud.length) aud.push('зрителям без жёстких предпочтений');
            var audience = 'Твоё, если ты — ' + aud.slice(0, 3).join('; ') + '.';

            return {
                pros: pros, cons: cons, audience: audience,
                tagline: tagline, review: rt, score: score, norm: norm,
                vClass: vClass, vWord: vWord, reason: reason, confidence: confidence,
                mode: dataRich ? 'TMDB' : 'TAGS'
            };
        });
    }

    /* ===== BACK / КОНТРОЛЛЕР (рабочая логика v19) ===== */
    ...
    function buildReadyInner(a) {
        ...
        var tagline = a.tagline ? '<div class="sw-tagline">' + esc(a.tagline) + '</div>' : '';
        var quote = '';
        if (a.review.sample) {
        ...
        return '' +
            '<div class="sw-dossier">' + badge +
                ...
                '<div class="sw-meter-scale"><span>не стоит</span><span>спорно</span><span>стоит</span></div>' +
                tagline +
            '</div>' +
            ...
            '</div>' +
            quote +
            '<div class="sw-target-audience"><div class="sw-title target">🎯 Кому зайдёт</div><div>' + esc(a.audience) + '</div></div>' +
            '<div class="sw-decision">' +
                '<div class="sw-decision-hint">Наш вердикт выше — а если всё равно колеблешься, доверься случаю</div>' +
    ...
    function showModal(movie) {
        ...
        var phrases = ['Подгружаю съёмочную группу…', 'Читаю отзывы зрителей…', 'Проверяю ценз и теги…', 'Взвешиваю аргументы…'];
        ...
        analyze(movie).then(function(a) {
            ...
            bindDice(html);
            // живое заполнение шкалы после появления
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v20.0 (reviews + tagline + weighted verdict).');
    }
