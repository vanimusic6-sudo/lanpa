(function () {
    ...
    /* ===== СЦЕНАРИИ: второй контур интерпретации тех же флагов ===== */
    function buildScenarios(f, runtime) {
        var S = [], seen = {};
        function add(ico, when, why) { if (seen[when]) return; seen[when] = 1; S.push({ ico: ico, when: when, why: why }); }
        if (f.nudity || f.sex) add('👶', 'не с детьми', 'в кадре нагота и сексуальные сцены');
        if (f.nudity || f.sex) add('🫣', 'не в смешанной компании', 'родители, коллеги или малознакомые — будет неловко');
        if (f.drugs || f.alcohol || f.smoke) add('🧠', 'не в уязвимом состоянии', 'темы зависимости могут задеть за живое');
        if (f.violence || f.hate) add('😴', 'не перед сном и не для фона', 'тяжёлые сцены требуют внимания, а не шума на кухне');
        if (f.profanity) add('🔇', 'не при чувствительных к мату', 'нецензурная лексика льётся потоком');
        if (runtime > 150) add('⏳', 'не когда в обрез по времени', 'хронометраж ' + runtime + ' мин — закладывай вечер');
        return S;
    }
