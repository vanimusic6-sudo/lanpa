(function () {
    ...
    /* ===== ДОГРУЗКА (теперь + английский overview) ===== */
    ...
    function loadMeta(movie) {
        ...
        return Promise.all([
            ...
            tmdbGet('/' + type + '/' + id + '?language=en-US')   // английский оригинал для детекции
        ]).then(function(arr) {
            var kw = [];
    ...
    function analyze(movie) {
        return Promise.all([ loadCredits(movie), loadMeta(movie) ]).then(function(arr) {
            ...
            var isCrimeDrama = hasGenre(genres, /crime|криминал/i) && hasGenre(genres, /drama|драма/i);

            // Расширенные флаги: читаем оба языка + keywords
            ...
            if (rating >= 8.0 && votes >= 2000 && yr > 0 && yr <= now - 3) addP('🏛 культовое кино', 18);
            if (yr >= now - 1 && votes >= 200) addP('🔥 на хайпе', 10);
            if (votes > 0 && votes < 30) addC('❓ оценок слишком мало', 8);

            if (rt.tone === 'pos') addP('💬 зрители в отзывах хвалят', 20);
            ...
            if (q && !/CAM|TS|HDCAM|SCR|WORKPRINT/i.test(q)) addP('🎥 качественное видео (' + (q || 'HD') + ')', 8);
            if (runtime > 0 && runtime <= 120) addP('🕐 можно уложиться в один вечер (' + runtime + ' мин.)', 6);
            ...
            if (fNudity) addC('🫣 откровеннbz,нагота', 14);
            ...
            if (runtime > 180) addC('⌛ сложно посмотреть за раз (' + runtime + ' мин.)', 10);
            ...
            // АНСАМБЛЕВЫЙ СИГНАЛ: высокий ценз без конкретных keywords = общий минус
            // Это честно: R/NC-17/MA по определению = есть хотя бы одно из: нагота/мат/насилие
            ...
            return {
                pros: pros, cons: cons, audTags: aud,
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v22.1 (english overview + ensemble age-signal).');
    }
