(function () {
    ...
    function initSettings() {
        ...
        [
            { name: 'api_key', type: 'input', title: 'Groq API-ключ (бесплатно)', description: 'console.groq.com/keys (начинается с gsk_...)', default: '' },
            { name: 'ai_mode', type: 'select', title: 'Режим анализа', values: {'hybrid':'ИИ + теги (рекомендуется)','ai_only':'Только ИИ','tags_only':'Только теги TMDB (без ИИ)'}, default: 'hybrid' },
    ...
    /* ==========================================================================
       СТИЛИ (живой лоадер + пульс индикатора + плавное появление)
       ========================================================================== */
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            ...
            '.sw-loader-text{font-size:1.15em;font-weight:600;letter-spacing:.02em;min-height:1.4em;transition:opacity .25s;color:#94a3b8}' +
            ...
            '.sw-mode-badge.ai{background:#f55036;color:#fff}' +
            '.sw-mode-badge.tags{background:rgba(255,255,255,.15);color:#ccc}' +
            '.sw-mode-badge.error{background:#7a2020;color:#ffd2d2}' +
            '.sw-mode-dot{width:8px;height:8px;border-radius:50%;display:inline-block}' +
            '.sw-mode-dot.active{background:#85c25e;animation:swPulse 1.4s ease-in-out infinite}' +
            ...
            '.sw-col{flex:1;background:rgba(255,255,255,.05);padding:15px;border-radius:10px;transition:background .2s}' +
            '.sw-col:hover{background:rgba(255,255,255,.07)}' +
            ...
            '.sw-list li{margin-bottom:10px;animation:swFade .4s ease both}' +
    ...
    // Округляет длинные дроби (7.591 -> 7.6), не трогает целые (125, 1080, 2024)
    function cleanNum(s) { return typeof s === 'string' ? s.replace(/\d+\.\d{2,}/g, function(m){ return parseFloat(m).toFixed(1); }) : s; }
    // Вырезает непослушные пункты-галлюцинации
    ...
    /* ==========================================================================
       GROQ API (промпт v2: факты + запрет галлюцинаций + эмодзи по смыслу)
       ========================================================================== */

    function analyzeWithGroq(movie, facts) {
        var key = getSettings().api_key;
        ...
        var systemPrompt =
            'Ты — язвительный, но точный кинокритик. Пишешь коротко и по делу, без воды и философии.\n' +
            'Отвечай СТРОГО одним JSON-объектом, без markdown, без ``` и без пояснений вне JSON.';
        ...
        var userPrompt =
            ...
            'Верни JSON вида:\n' +
            '{"pros":[...],"cons":[...],"audience":"..."}\n\n' +
            'ПРАВИЛА ДЛЯ pros (3-6 пунктов):\n' +
            '- Каждый пункт = один факт, 4-9 слов, с эмодзи в начале.\n' +
            '- Рейтинг пиши ТОЛЬКО как ' + rating + ' (одна цифра после точки).\n' +
            '- Если в ФАКТАХ есть режиссёр/актёры/хронометраж/ценз — используй их по именам и цифрам.\n\n' +
            'ПРАВИЛА ДЛЯ cons (0-6 пунктов):\n' +
            '- Проверяй по синопсису и жанрам: насилие, наркотики, курение/алкоголь, нагота, жестокость, азартные игры, ненависть, плохое качество (CAM/TS), слишком длинный хронометраж (>180 мин), взрослый ценз (16+/18+).\n' +
            '- Для криминала/триллера/ужасов и тем про наркотики (метамфетамин и т.п.) cons ОБЯЗАТЕЛЬНО не пустой.\n' +
            '- Мультфильм/анимация НЕ делает фильм семейным, если есть взрослый ценз или темы насилия/наркотиков.\n\n' +
            'ПРАВИЛО audience: одно предложение, начинается со слова "Любителям", перечисление через запятую, без воды.\n\n' +
            'ЭМОДЗИ — подбери САМ по смыслу пункта из словаря:\n' +
            '⭐ рейтинг | 🎬 режиссёр | ✍️ сценарист | 🎭 актёры | 🎵 музыка | 💥 экшен | 🎥 качество картинки | 🕐 хронометраж | 🆕 новинка | 🔎 признание/топ | 🤱 семейный | 🦫 познавательный | 🌍 страны/разнообразие | 🔗 франшиза | 💋 откровенность | 🚬 курение/алкоголь | 🎰 азартные игры | 🔪 насилие | 💉 наркотики | 📺 плохое качество | 🚩 ненависть | 🔞 возрастное ограничение | ⌛ слишком длинный.\n' +
            'ЗАПРЕТЫ: не используй эмодзи ⛔ (зарезервирован). Не повторяй один эмодзи дважды в одном списке.\n\n' +
            'ГЛАВНЫЙ ЗАПРЕТ: если факта нет в списке ИЗВЕСТНЫЕ ФАКТЫ — НЕ создавай пункт про него и НИКОГДА не пиши слова "не указан", "нет данных", "неизвестно", "потенциально", "возможно". Лучше пропусти пункт.\n\n' +
            'АНТИ-ПРИМЕРЫ (так писать НЕЛЬЗЯ):\n' +
            '❌ "🎬 режиссёр: не указан"\n' +
            '❌ "⌛ потенциально высокий хронометраж"\n' +
            '❌ "возможность для зрителей задуматься о своих представлениях о мире"\n' +
            '❌ "интересный сюжет", "хорошая игра актёров", "высокий рейтинг (7.591/10)"\n\n' +
            'ЭТАЛОН (фильм "Джокер", рейтинг 8.4, режиссёр Тодд Филлипс, в ролях Хоакин Феникс, 122 мин, ценз 18+):\n' +
            '{"pros":["⭐ высокий рейтинг (8.4)","🎬 режиссёр: Тодд Филлипс","🎭 Хоакин Феникс в главной роли","🕐 комфортный хронометраж (122 мин)"],' +
            '"cons":["🔪 сцены насилия","💉 тема психического расстройства","🔞 ценз 18+ — не для детей"],' +
            '"audience":"Любителям мрачных драм и бенефисов одного актёра."}';
                ...
                .then(function(data) {
                    try {
                        if (data && data.choices && data.choices[0] && data.choices[0].message) {
                            ...
                            res(json);
                        } else { console.error('[SW] Groq response error:', data); res(null); }
                    } catch(e) { console.error('[SW] Groq parse error:', e); res(null); }
                })
                .catch(function(err){ console.error('[SW] Groq fetch error:', err); res(null); });
            } catch(e) { console.error('[SW] Groq error:', e); res(null); }
        });
    }
    ...
    function analyze(movie) {
        var cfg = getSettings();
        var useAI = cfg.api_key && (cfg.ai_mode === 'hybrid' || cfg.ai_mode === 'ai_only');
        ...
        return Promise.all([ loadCredits(movie), loadExtra(movie) ]).then(function(arr) {
            ...
            // Блок фактов для ИИ (только то, что реально есть)
            var facts = [];
            ...
            var aiPromise = useAI ? analyzeWithGroq(movie, facts) : Promise.resolve(null);
            ...
            return aiPromise.then(function(aiResult) {
                ...
                if (aiResult && Array.isArray(aiResult.pros) && Array.isArray(aiResult.cons)) {
                    ...
                    var audience = cleanNum(aiResult.audience) || 'Любителям кино.';
                ...
                var mG2 = genres.filter(function(g){ var gl = g.toLowerCase(); return blG.some(function(b){ return gl.indexOf(b) >= 0; }); });
                var mA2 = cast.filter(function(a){ var al = a.toLowerCase(); return blA.some(function(b){ return al.indexOf(b) >= 0; }); });
                var mD2 = [].concat(dirs, wrts).filter(function(p){ var pl = p.toLowerCase(); return blD.some(function(b){ return pl.indexOf(b) >= 0; }); });
    ...
    /* ==========================================================================
       МОДАЛКА (живой лоадер -> плавная подстановка, один open)
       ========================================================================== */

    function buildReadyInner(a) {
        var badge = '';
        if (a.mode === 'AI') badge = '<span class="sw-mode-badge ai"><span class="sw-mode-dot active"></span>GROQ</span>';
        else if (a.mode === 'AI_ERROR') badge = '<span class="sw-mode-badge error"><span class="sw-mode-dot inactive"></span>AI (ошибка)</span>';
        else badge = '<span class="sw-mode-badge tags"><span class="sw-mode-dot inactive"></span>TAGS</span>';
        ...
        return '' +
            ...
            '<div class="sw-columns">' +
                '<div class="sw-col"><div class="sw-title pros">Почему стоит ✓</div><ul class="sw-list">' + a.pros.map(function(p, i){ return '<li style="animation-delay:' + (i * 0.05) + 's">' + esc(p) + '</li>'; }).join('') + '</ul></div>' +
                '<div class="sw-col"><div class="sw-title cons">Почему не стоит ✗</div><ul class="sw-list">' + a.cons.map(function(c, i){ return '<li style="animation-delay:' + (i * 0.05) + 's">' + esc(c) + '</li>'; }).join('') + '</ul></div>' +
            '</div>' +
    ...
    function showModal(movie) {
        ...
        // Живой лоадер внутри модалки (модалка открывается ОДИН раз -> Back не ломается)
        var loaderPhrases = ['Собираю факты о фильме…', 'Читаю синопсис и отзывы…', 'Спрашиваю ИИ-критика…', 'Формирую вердикт…'];
        var html = $(
            '<div class="sw-modal-content">' +
                '<div id="sw-body">' +
                    '<div class="sw-loader">' +
                        '<div class="sw-loader-dice">🎲</div>' +
                        '<div class="sw-loader-text" id="sw-loader-text">' + loaderPhrases[0] + '</div>' +
        ...
        window._sw_loaderTimer = setInterval(function() {
            pi = (pi + 1) % loaderPhrases.length;
            var t = html.find('#sw-loader-text');
            if (t.length) { t.css('opacity', 0); setTimeout(function(){ t.text(loaderPhrases[pi]).css('opacity', 1); }, 200); }
        }, 800);
        ...
        analyze(movie).then(function(a) {
            if (window._sw_loaderTimer) { clearInterval(window._sw_loaderTimer); window._sw_loaderTimer = null; }
            // Подстановка результата в тот же DOM (без Modal.update)
            var body = html.find('#sw-body');
            body.html('<div class="sw-body">' + buildReadyInner(a) + '</div>');
            bindDice(html);
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v13.0 initialized (facts-fed prompt + live loader).');
    }
