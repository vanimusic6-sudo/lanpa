(function () {
    ...
    function analyze(movie) {
        return Promise.all([loadCredits(movie), loadMeta(movie), Promise.resolve(lampaLocal(movie)), loadKp(movie)]).then(function(arr){
            ...
            /* база измерений: только целые словоформы, без обрывков */
            ...
            /* возраст: TMDB -> KP -> карточка Lampa -> вывод по содержанию (только при уверенности) */
            var age = meta.age;
            var ageInferred = false;
            if (age === null && kp && kp.age !== null && kp.age !== undefined) age = kp.age;
            if (age === null && dom.age) age = dom.age;
            if (age === null && (cSex >= 50 || cDrugs >= 50 || cViol >= 50)) { age = 16; ageInferred = true; }
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v20.0 (clean wordforms, no false positives)');
    }
