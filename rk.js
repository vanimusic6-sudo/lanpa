(function () {
    ...
    /* Стабильные ID жанров TMDB (не зависят от языка перевода) */
    var GENRE_ID_ANIM   = 16;     // Animation
    var GENRE_ID_FAMILY = 10751;  // Family
    var GENRE_ID_KIDS   = 10762;  // Kids (TV)
    ...
    function saveSetting(k, v) { try { Lampa.SettingsApi && Lampa.Storage.set(PLUGIN_ID + '_' + k, v); } catch(e) {} }
    ...
    /* ===== ДЕТЕКТОР ЖАНРОВ ПО ID + ИМЕНИ (работает без доп. запросов TMDB) ===== */
    // Элемент genresRaw может быть объектом {id,name} или строкой — учитываем оба.
    ...
    function initNavPoints(html) {
        try {
            ...
            if (middlePoint.length) { middlePoint.addClass('sw-nav-point'); window._sw_navPoints.push(middlePoint); }
            var bottomPoint = html.find('.sw-decision');
            if (bottomPoint.length) { bottomPoint.addClass('sw-nav-point'); window._sw_navPoints.push(bottomPoint); }
    ...
    function scrollToNavPoint(index) {
        try {
            ...
            if (point && point.length) {
                try { point[0].scrollIntoView({ behavior: 'smooth', block: 'center' }); }
                catch(e) { var c = window._sw_scrollContainer; if (c && c.length) c[0].scrollTop = point[0].offsetTop - 20; }
                setTimeout(function() {
                    try {
                        var ft = point.find('.sw-focusable').first(); if (!ft.length) ft = point;
                        window._sw_navPoints.forEach(function(p){ p.find('.sw-focusable').removeClass('focus'); p.removeClass('focus'); });
                        ft.addClass('focus');
    ...
    function loadMeta(movie) {
        ...
        return Promise.all([
            ...
            tmdbGet('/' + type + '/' + id, 'en-US') // описание именно на EN — для англ. сигналов
        ]).then(function(arr) {
            var kw = [];
    ...
    /* ===== АУДИТОРИЯ (без хард-кода названий) ===== */
    ...
    function analyze(movie) {
        return Promise.all([ loadCredits(movie), loadMeta(movie) ]).then(function(arr) {
            ...
            var genresRaw = movie.genres || [];                       // объекты {id,name} или строки
            ...
            /* --- Детектор по жанрам (работает даже в TAGS-режиме, т.к. жанры есть в карточке) --- */
            ...
            /* ===== НОВАЯ ЛОГИКА СЕМЕЙНОСТИ =====
               Для анимации главный сигнал — жанр Family/Kids (есть в карточке всегда).
               Явный взрослый ценз (16+) или взрослые темы — перебивают жанр.
               Для не-анимации Family-жанр тоже усиливает семейность. */
            var familyOK;
            if (isAnim) {
                if (hardAdult) familyOK = false;                       // явные взрослые темы — никогда не семейный
                else if (age !== null && age >= 16) familyOK = false;  // взрослый ценз перебивает
                else if (hasFamilyGenre) familyOK = true;              // ★ главный сигнал (MLP, Elemental, Frozen...)
                else if (age !== null && age <= 12) familyOK = true;   // детский ценз
                else if (kidsKw) familyOK = true;                      // ключевые слова TMDB
                else if (age !== null && age > 12) familyOK = false;   // ценз 13-15 без Family-жанра
                else familyOK = false;                                 // анимация без Family-жанра и без данных → скорее взрослая
            } else {
            ...
            // для анимации «актёрская игра» некорректна — даём озвучку/историю
            ...
            // взрослая анимация: формулировка зависит от того, есть ли ценз/темы
            if (isAnim && !familyOK) {
    ...
    function registerController() {
        try {
            Lampa.Controller.add('should_watch_modal', {
                ...
                up: function() { if (window._sw_blocknav) { if (window._sw_navPoints && window._sw_navPoints.length > 1) scrollToNavPoint(window._sw_currentNavPoint - 1); else swSetFocus((window._sw_focusIdx || 0) - 1); } },
                down: function() { if (window._sw_blocknav) { if (window._sw_navPoints && window._sw_navPoints.length > 1) scrollToNavPoint(window._sw_currentNavPoint + 1); else swSetFocus((window._sw_focusIdx || 0) + 1); } },
    ...
    function showModal(movie) {
        try {
            ...
            window._sw_currentModalHtml = html; window._sw_focusIdx = 0; window._sw_scrollContainer = html;
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v26.0 (genre-ID family detection, no hardcoded list).');
    }
