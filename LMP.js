(function () {
    ...
    /* ===== СТИЛИ (убрал мусор: .sw-btn-close больше нет) ===== */
    function injectCSS() {
        try {
    ...
    /* ===== БЛОЧНАЯ НАВИГАЦИЯ (с защитой от пустого контейнера) ===== */
    ...
    function tmdbGet(path) {
        return new Promise(function(res) {
            try {
                ...
                fetch('https://api.themoviedb.org/3' + path + sep + 'api_key=' + tmdbKey())
    ...
    function loadMeta(movie) {
        ...
        return Promise.all([
            ...
            tmdbGet(ratingPath),
            tmdbGet('/' + type + '/' + id + '/reviews?language=ru-RU'),
            tmdbGet('/' + type + '/' + id + '/reviews?language=en-US'),
            tmdbGet('/' + type + '/' + id + '/videos?language=ru-RU'),
            tmdbGet('/' + type + '/' + id + '/videos?language=en-US'),
            ...
            if (arr[2] && arr[2].results) reviews = reviews.concat(arr[2].results.slice(0, 5).map(parseRev));
            if (arr[3] && arr[3].results) reviews = reviews.concat(arr[3].results.slice(0, 5).map(parseRev));
            reviews = reviews.filter(function(r){ return r.text.length > 20; });
            ...
            var hasTrailer = checkTrailer(arr[4]) || checkTrailer(arr[5]);

            var enOv = (arr[6] && arr[6].overview) ? arr[6].overview : '';
    ...
    /* ===== BACK / КОНТРОЛЛЕР (регистрируется ОДИН раз при старте) ===== */
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v23.0 initialized (Lampa canon).');
    }
