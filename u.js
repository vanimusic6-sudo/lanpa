(function () {
    ...
    var KNOWN_FAMILY_MOVIES = [
        ...
        'моана', 'moana',
        'зоотопия', 'zootopia',
        'коко', 'там где нас нет', 'coco',
        'душа', 'soul',
        'лука', 'luca',
        'вперёд', 'onward',
        'энканто', 'encanto',
        'краснеет', 'turning red',
        'лайтйир', 'lightyear',
        'элементарно', 'elemental',
        'человек-паук: через вселенные', 'spider-verse',
        'семейка митчелов против машин', 'mitchells vs the machines',
        'шрек', 'shrek',
    ...
    /* ===== АНАЛИЗ (ИСПРАВЛЕННАЯ ЛОГИКА ДЛЯ МУЛЬТФИЛЬМОВ) ===== */
    function analyze(movie) {
        return Promise.all([ loadCredits(movie), loadMeta(movie) ]).then(function(arr) {
            ...
            // Определение взрослого контента
            ...
            // ОБНОВЛЁННАЯ ЛОГИКА: Для мультфильмов ориентируемся по возрастному рейтингу
            // Если мультфильм имеет ценз <= 12, то он семейный, независимо от других факторов
            // Если ценз > 12 или неизвестен, но есть взрослые темы, то не семейный
            // Если ценз неизвестен, но это известный детский фильм, то семейный
            
            var anyAdult = fDrugs || fNudity || fSex || fViol || fSmoke || fAlcohol || fProfanity || fHate || fGamb || fAdultAnim || fSuicide || !!movie.adult;
            
            // Для анимации: ориентируемся по рейтингу
            var familyOK;
            if (isAnim) {
                // Если ценз известен и <= 12, то семейный
                if (age !== null && age <= 12) {
                    familyOK = true;
                }
                // Если ценз > 12, то не семейный
                else if (age !== null && age > 12) {
                    familyOK = false;
                }
                // Если ценз неизвестен, но это известный детский фильм
                else if (isKnownFamily) {
                    familyOK = true;
                }
                // Если ценз неизвестен, но есть детские ключевые слова
                else if (kidsKw) {
                    familyOK = true;
                }
                // Во всех остальных случаях для анимации без известного ценза - не семейный
                // (потому что недетские мульты обычно имеют ценз 16+ или выше)
                else {
                    familyOK = false;
                }
            } else {
                // Для обычных фильмов: стандартная логика
                familyOK = !anyAdult && rating >= 5 && ((age !== null && age <= 12) || kidsKw || isKnownFamily);
            }
            
            // Дополнительная проверка: если это известный детский фильм, но что-то пошло не так
            ...
            // Для анимации, которая не прошла как семейная
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v25.0 (TV navigation + rating-based family detection for cartoons).');
    }
