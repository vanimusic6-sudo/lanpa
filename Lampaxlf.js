(function () {
    ...
    function injectCSS() {
        try {
            ...
            s.innerHTML =
                ...
                '.sw-btn-close{margin-top:20px; text-align:center; padding: 15px; background: rgba(255,255,255,0.1); border-radius: 12px; cursor: pointer}' +
    ...
    Lampa.Controller.add(PLUGIN_ID, {
        ...
        up: function () {
            if (window._sw_focusIdx > 0) {
                ...
                // Если мы на самом верху, даем возможность прокрутить сам блок
        ...
        down: function () {
            ...
            if (window._sw_focusIdx < blocks.length - 1) {
                ...
                // Магия для пульта: если кнопок больше нет, скроллим контейнер вниз принудительно
                var el = $('.sw-modal-content')[0];
                if (el) el.scrollTop += 100;
            }
        },
    ...
    function analyze(movie) {
        return Promise.all([ loadCredits(movie), loadMeta(movie) ]).then(function(arr) {
            ...
            // Фиксы галлюцинаций: добавлены границы слов \b для EN и улучшены проверки RU
            ...
            // Переписано в стиле "Ориентировка"
    ...
    function buildModal(movie) {
        if (window._sw_currentModalHtml) window._sw_currentModalHtml.remove();
        var wrap = $('<div class="sw-modal-content"></div>');
        wrap.append('<div class="sw-loader"><div class="sw-loader-dice">🎲</div><div class="sw-loader-text">Собираем досье на объект...</div><div class="sw-loader-bar"></div></div>');
        
        $('body').append(wrap);
        window._sw_currentModalHtml = wrap;
        ...
        analyze(movie).then(function(data) {
            ...
            html += '<div class="sw-col"><div class="sw-title pros">🟢 Выявлены преимущества</div><ul class="sw-list">';
            data.pros.forEach(function(p){ html += '<li>' + esc(p) + '</li>'; });
            html += '</ul></div>';

            html += '<div class="sw-col"><div class="sw-title cons">🔴 Обнаружены риски</div><ul class="sw-list">';
            data.cons.forEach(function(c){ html += '<li>' + esc(c) + '</li>'; });
            html += '</ul></div>';
            html += '</div>';

            // Аудитория
            html += '<div class="sw-target-audience"><div class="sw-title target">👤 Психологический профиль зрителя</div><div class="sw-aud-text">' + esc(data.audience) + '</div></div>';

            // Кнопка закрытия
            html += '<div class="sw-btn-close sw-focusable focus">ЗАКРЫТЬ ДОСЬЕ</div>';
            html += '</div>';

            wrap.append(html);
            
            wrap.find('.sw-btn-close').on('hover:enter click', function() {
                restorePrev();
            });
