(function () {
    ...
    function hasAdultTags(ctx) {
        if (!ctx || !ctx.kw || !ctx.kw.length) return false;
        var adultKeywords = ['adult', 'mature', '18+', '16+', '14+', 'r rated', 'nc-17', 'x rated', 'взрослый', 'для взрослых', '18+', '16+', 'не для детей', 'dark comedy', 'black comedy', 'adult animation', 'satire', 'дysfunctional', 'тёмная комедия', 'чёрный юмор', 'для взрослых', 'сатира'];
        return ctx.kw.some(function(kw) {
            return adultKeywords.some(function(tag) {
                return kw.toLowerCase().indexOf(tag) >= 0;
            });
        });
    }
    ...
    function analyze(movie) {
        return Promise.all([ loadCredits(movie), loadMeta(movie) ]).then(function(arr) {
            ...
            var adultKw = hasKw(ctx, /adult|mature|18\+|16\+|r rated|взрослый|для взрослых|не для детей/i) || hasAdultTags(ctx);

            var movieTitle = movie.title || '';
            var movieOriginalTitle = movie.original_title || movie.original_name || '';

            // === ЛОГИКА ОПРЕДЕЛЕНИЯ СЕМЕЙНОГО КОНТЕНТА ===
            var familyOK;

            // 1. Проверяем по возрастному рейтингу
            if (age !== null) {
                if (age <= 12) familyOK = true;
                else if (age <= 16) familyOK = !adultKw;
                else familyOK = false;
            }
            // 2. Проверяем по тегам
            else if (kidsKw) familyOK = true;
            // 3. Для анимации
            else if (isAnim) familyOK = !adultKw;
            // 4. Проверяем по жанрам
            else if (hasGenre(genres, /family|семейный|детский/i)) familyOK = true;
            else familyOK = false;

            // Дополнительная проверка для анимации с возрастным рейтингом > 12
            if (isAnim && age !== null && age > 12) familyOK = false;

            // Если включен режим семейного просмотра и film не семейный
            ...
            var fDrugs    = inAnyText(ovBoth, /метамфетамин|нарко|кокаин|героин|марихуан|амфетамин/i) || hasKw(ctx, /drug|narcotic|meth|cocaine|heroin/) || adultKw;
            var fNudity   = inAnyText(ovBoth, /обнаж|нагот|голы|эротик/i) || hasKw(ctx, /nudity|female nudity|male nudity/) || adultKw;
            var fSex      = inAnyText(ovBoth, /sex|эротик|откровен|оргазм|проститутк|интимн/i) || hasKw(ctx, /sex scene|sexual content|erotic/) || !!movie.adult || adultKw;
            var fViol     = inAnyText(ovBoth, /violenc|gore|murder|убийств|кров|жесток|насил|оружи|стрельб/i) || hasKw(ctx, /violenc|gore|murder|blood|weapon/) || hasGenre(genres, /horror|ужас/i) || adultKw;
            ...
            var anyAdult = fDrugs || fNudity || fSex || fViol || fSmoke || fAlcohol || fProfanity || fHate || fGamb || fSuicide || !!movie.adult || adultKw;

            if (isAnim && anyAdult) familyOK = false;
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v26.0 (TV/Phone scroll + Smart family detection).');
    }
