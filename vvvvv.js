(function () {
    ...
    /* ===== КИНОПОИСК: два эндпоинта, честный статус ===== */
    function kpGet(url) {
        return new Promise(function(res){
            try {
    ...
    function loadKp(movie) {
        var tok = getSettings().kp_token;
        if (!tok) return Promise.resolve(null);
        var title = (movie.title || movie.name || '').trim();
        if (!title) return Promise.resolve({ reviews: null, status: 'fail' });
        var enc = encodeURIComponent(title);
        function dev() {
            return kpGet('https://api.kinopoisk.dev/v1.4/movie/search?query=' + enc + '&limit=1').then(function(s){
                if (!s || !s.docs || !s.docs.length) return null;
                var id = s.docs[0].id || s.docs[0].kinopoiskId;
                if (!id) return { reviews: null, status: 'ok' };
                return kpGet('https://api.kinopoisk.dev/v1.3/review?filmId=' + id + '&page=1&limit=8').then(function(r){
                    ...
                    return { reviews: list, status: 'ok' };
                });
            });
        }
        function uns() {
            return kpGet('https://kinopoiskapi.unofficial.tech/api/v2.2/films/search-by-keyword?keyword=' + enc + '&page=1').then(function(s){
                var films = s && (s.films || s.items);
                if (!films || !films.length) return null;
                var id = films[0].filmId || films[0].id;
                if (!id) return { reviews: null, status: 'ok' };
                return kpGet('https://kinopoiskapi.unofficial.tech/api/v2.2/films/' + id + '/reviews?page=1').then(function(r){
                    var list = null;
                    var arr = r && (r.reviews || r.items);
                    if (arr && arr.length) {
                        list = arr.map(function(x){ return { author: x.author || 'Аноним', text: String(x.reviewText || x.review || x.title || '').trim() }; }).filter(function(x){ return x.text.length > 20; });
                        if (!list.length) list = null;
                    }
                    return { reviews: list, status: 'ok' };
                });
        ...
        return dev().then(function(r){ if (r) return r; return uns(); })
            .catch(function(){ try { return uns(); } catch(e){ return null; } })
            .then(function(r){ return r || { reviews: null, status: 'fail' }; });
    }

    /* ===== DOM-сигналы карточки: возраст (макс), настроения, комментарии ===== */
    function readDomSignals() {
        var out = { mm: {}, moods: [], ok: false, age: null, reviews: [] };
        try {
            var txt = document.body.innerText || '';
            var amAll = txt.match(/\b(6|12|16|18)\+/g);
            ...
            if (ci > -1) {
                ...
                for (var i2 = 0; i2 < lines.length && out.reviews.length < 6; i2++) {
                    ...
                    if (!ln) continue;
                    if (/^(Сезон|Режиссер|Актеры|Производство|Тэги|Теги|Настроения|Подробно)/.test(ln)) break;
    ...
    function analyze(movie) {
        return Promise.all([loadCredits(movie), loadMeta(movie), Promise.resolve(lampaLocal(movie)), loadKp(movie)]).then(function(arr){
            ...
            var allRev = meta.reviews.concat(dom.reviews || []);
            var kpUsed = false;
            if (kp && kp.reviews && kp.reviews.length) { allRev = allRev.concat(kp.reviews); kpUsed = true; }
            var rt = reviewStats(allRev);
            rt.src = kpUsed ? ' (TMDB + Кинопоиск)' : ((dom.reviews || []).length ? ' (по комментариям)' : '');
            var ctx = { kw: meta.kw };
            var M = 150, C = 6.1;
            ...
            var dIll = dim(/terminal illness|cancer|disease|hospital/, /онколог|болезн|больниц|диагноз|терап|инвалид|злокачествен/);
            ...
            var familyOK;
            if (isAnim) familyOK = !hardAdult && cSuic < 40 && cProf < 40 && (age === null || age <= 12) && (hasFamilyGenre || kidsKw || (age !== null && age <= 12));
            ...
            else if (age !== null && age >= 16) add('tmdb','con','🔞 Не для детей (' + age + '+)', 12, 'age');
            if (isAnim && !familyOK) add('tmdb','con','🎭 Анимация для взрослой аудитории' + (age !== null ? ' (' + age + '+)' : ''), 14, 'adultanim');
            ...
            if (rt.total >= 2 && rt.tone === 'pos') add('reviews','pro','💬 Зрители в восторге' + rt.src + ': ' + rt.pos + ' из ' + rt.total + ' отзывов положительные', 20, 'rev');
            else if (rt.total >= 2 && rt.tone === 'neg') add('reviews','con','💬 Может разочаровать' + rt.src + ': ' + rt.neg + ' из ' + rt.total + ' отзывов отрицательные', 22, 'rev');
            else if (rt.total >= 2 && rt.tone === 'mix') { add('reviews','pro','💬 Отзывы полярные' + rt.src + ': ' + rt.pos + ' из ' + rt.total + ' положительных', 6, 'rev'); add('reviews','con','💬 Часть зрителей осталась недовольна', 8); }
            ...
            var activeSrc = 1 + (metaRich ? 1 : 0) + (rt.total >= 2 ? 1 : 0) + (dom.ok || moods.length ? 1 : 0) + (local.inFavorite || local.viewedPercent > 0 ? 1 : 0) + (kpUsed ? 1 : 0);
    ...
    function buildReadyInner(a) {
        ...
        if (a.review.sample && a.review.total >= 2) {
            ...
            quote = '<div class="sw-quote' + toneCls + '"><div class="sw-quote-text">«' + esc(txt) + '»</div><div class="sw-quote-meta">— ' + esc(a.review.sample.author) + ', отзыв зрителя' + (a.review.src || '') + '</div></div>';
        }
        return '' +
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v21.0 (card-only age, card comments, kp fallback)');
    }
